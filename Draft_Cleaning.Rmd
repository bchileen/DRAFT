...
pools that are not of interest, clean up the SURVEYJOBIDPK, and create some 
...
dredge <-gage_survey_cleaned |>
  filter(DREDGINGPURPOSE != "None")
  hist(dredge$VOLUMEDREDGED)
  
  median(dredge$VOLUMEDREDGED)

hist(gage_survey_cleaned$VOLUMEDREDGED)

plot(dredge_PCA,  type="l")

autoplot(dredge_PCA,data = gage_survey_cleaned, 
