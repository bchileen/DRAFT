...
fit_dredge = train(VOLUMEDREDGED ~ .,
                 data = gage_xgb,
                 method = "xgbTree",
                 tuneGrid = expand.grid(nrounds = c (15,25,50,100), max_depth = 1:10,
                                        eta = 0.3, gamma = 0,
                                        colsample_bytree = .8,
...
fit_dredge2 = train(VOLUMEDREDGED ~ .,
                   data = gage_xgb,
                   method = "xgbTree",
                   tuneGrid = tune_grid,
                   verbosity = 0,
...
# Model evaluation
predictions <- fit_dredge2$pred  # Predictions from cross-validation
head(predictions)

# RMSE, R-Squared, etc.
cv_rmse <- sqrt(mean((predictions$obs - predictions$pred)^2))
cv_r2 <- cor(predictions$obs, predictions$pred)^2
