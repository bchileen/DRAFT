...
summary(gage_data)
glimpse(gage_data)
na_counts <- colSums(is.na(gage_data))

names<-names(gage_data)
...
na_counts_post <- colSums(is.na(gage_data))

names<-names(gage_data)
# Create a data frame for the table
na_table <- data.frame(
  Gage_Name = names,
  NA_Count = na_counts_post
)|>
  filter(NA_Count >=1)
```
...
1999 to 2024, these are going to act as my background data. I want to combine
...
dredge_data <- dredge_data |>
  ...
  select(EXECYEAR, VOLUMEDREDGED, RIVER,POOL,
...
CSAT_Data<-combined_data_full_clean |>
  mutate(DATE_START = as.character(SurveyDateAfter),
         DATE_START = as_date(DATE_START))
  
gage_survey_CSAT <- gage_survey_full|>
  left_join(CSAT_Data, by = c("DATE_START","POOL" = "pool"))
...
gage_scaled_clean <- gage_scaled_clean 
