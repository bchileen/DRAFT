...
ggplot(combined_data_clean, aes(x = After_date, y = AnnualShoalingRate_ftperyr, colour = river)) +
  geom_point() + 
  geom_line() +
  geom_hline(yintercept = 30, color = "goldenrod", linetype = "dashed", linewidth = 1) +
  labs(title = "Annual Shoaling Rate (ft/year)",
       x = "Date of Post Survey",
       y = "Annual Shoaling Rate",
       color = "River") +
  facet_wrap(~ river, labeller = labeller(river = c("IL" = "Illinois Waterway", 
                                                      "UM" = "Mississippi River"))) +
  scale_colour_discrete(labels = c("IL" = "Illinois Waterway", 
                                    "UM" = "Mississippi River"))
...
combined_data_full_clean <- combined_data_year |>
  filter(complete.cases(combined_data_year))
```
