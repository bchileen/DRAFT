...
def get_valid_coordinates(ds):
    """Extract valid coordinates with enhanced filtering"""
    logger = logging.getLogger()
    ...
    # Check for CSAT-style valid_range attributes
    if hasattr(ds['latitudes'], 'valid_range'):
        lat_range = ds['latitudes'].valid_range
        lat_valid = (lats >= lat_range.min()) & (lats <= lat_range.max())
        valid_mask = valid_mask & lat_valid
        logger.info(f"Latitude valid_range applied: {lat_valid.sum()} points")

    if hasattr(ds['longitudes'], 'valid_range'):
        lon_range = ds['longitudes'].valid_range
        lon_valid = (lons >= lon_range.min()) & (lons <= lon_range.max())
        valid_mask = valid_mask & lon_valid
        logger.info(f"Longitude valid_range applied: {lon_valid.sum()} points")

    final_count = valid_mask.sum()
    logger.info(f"Final valid coordinates: {final_count} / {len(lats)}")
...
def clip_netcdf_hybrid(nc_file, boundary, output_dir, reach_table):
    """Hybrid clipping with reach-specific CRS handling"""
    logger = logging.getLogger()
    ...
    try:
        ...
        valid_lats, valid_lons, valid_mask = get_valid_coordinates(ds)

        # Transform boundary to match NetCDF coordinate system (key insight from script 2)
        logger.info(f"Transforming boundary from {boundary.crs} to {source_crs}")
        boundary_reproj = boundary.to_crs(source_crs)

        # Create points in the NetCDF's coordinate system
        # Note: treating lat/lon variables as x/y coordinates (might be State Plane)
        points = [Point(lon, lat) for lon, lat in zip(valid_lons, valid_lats)]
        points_gdf = gpd.GeoDataFrame(geometry=points, crs=source_crs)

        # Use spatial indexing for performance (from script 2)
        logger.info("Creating spatial index for efficient clipping...")
        spatial_index = points_gdf.sindex

        # Find points within boundary using spatial index
        mask = np.zeros(len(points_gdf), dtype=bool)
        for poly in boundary_reproj.geometry:
            ...
            if possible_matches_index:
                ...
                precise_matches = possible_matches.intersects(poly)
                mask[possible_matches_index] |= precise_matches

        within_count = mask.sum()
        total_count = len(valid_lats)
        ...
        if within_count == 0:
            ...
            extent = (valid_lons.min(), valid_lats.min(), valid_lons.max(), valid_lats.max())
            create_no_overlap_plot(nc_file, boundary_reproj, extent, output_dir, source_crs)
            return None, extent

        # Map back to original dataset indices
        original_indices = np.where(valid_mask)[0]
        clipped_indices = original_indices[mask]

        # Create clipped NetCDF file using netCDF4 approach (from script 2)
        ...
        with nc.Dataset(nc_file, 'r') as src, nc.Dataset(output_file, 'w') as dst:
            ...
            dst.setncattr('points_before_clipping', str(len(ds['latitudes'])))
                    ...
                    if 'points' in variable.dimensions:
                        if len(variable.dimensions) == 1:
                            ...
                            if hasattr(data, 'mask'):
                                masked_data = data[clipped_indices]
                                fill_value = variable._FillValue if hasattr(variable, '_FillValue') else -9999
                                dst[name][:] = np.ma.filled(masked_data, fill_value=fill_value)
                            else:
